...
module Wtf
  class XCodeBuild 
    ...
    def self.render_define_key k
      k.to_s.upcase
    end
    
    def self.render_define_value v
      #TODO: throw error if unknown type
      case v
      when TrueClass
        'YES'
      when FalseClass
    ...
    def self.render_define_pair pair
      k, v = pair
      "#{self.render_define_key k}=#{self.render_define_value v}"
    end

    def self.render_defines defines
      defines.map { |p| self.render_define_pair p }.join " "
    end
    ...
    def self.run action:"", options:{}, defines:nil, dir:'.', log_suffix:nil
      ...
      to_run = "cd #{dir}; rvm system do #{BIN} #{self.render_options options} #{self.render_defines defines} #{action} > '#{logfile}'"
      Wtf.log.info to_run
    ...
    def self.export_archive project, archive, export_options=nil
      ...
      stdout, status = self.run(options: options, dir: project, log_suffix:"export_archive")
